...
$Root = $PSScriptRoot
$ReleaseDir = Join-Path $Root "build\release-win2008"
$OutputFile = Join-Path $ReleaseDir "webdown.exe"

if ([string]::IsNullOrWhiteSpace($GoCommand)) {
  $Go120 = Get-Command go1.20.14 -ErrorAction SilentlyContinue
  if ($Go120) { $GoCommand = $Go120.Source }
  else { $GoCommand = "go" }
}

$GoVersion = (& $GoCommand version)
if ($GoVersion -notmatch "go1\.20\.") {
  Write-Host "Current Go version: $GoVersion" -ForegroundColor Yellow
  Write-Host "Windows Server 2008 requires a binary built by Go 1.20.x." -ForegroundColor Yellow
  Write-Host "Please install Go 1.20.14 or Go 1.20.x, then run this script again." -ForegroundColor Yellow
  Write-Host "If you installed the downloader, run: go1.20.14 download" -ForegroundColor Yellow
  exit 1
}
...
try {
  ...
  $env:GOOS = "windows"
  $env:GOARCH = "amd64"
  & $GoCommand build -o $OutputFile .\cmd\webdown
...
Set-Content -NoNewline -Encoding ascii $ReleaseRunBat -Value $RunBat
  ...
  Write-Host "This package is built by $GoVersion for Windows Server 2008."
  Write-Host "Copy release-win2008 to the server."
  Write-Host "  - On Windows Server 2008: run.bat"
  Write-Host "  - On Linux (if needed) : run.sh / stop.sh / install-service.sh"
}
finally {
  Pop-Location
}
